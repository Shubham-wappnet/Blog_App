...
const addBlog = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Blog is uploaded successfully" })
...
const getByUserId = async (req, res) => {
    ...
    try {
        const userBlog = await User.findById(userId).select(' name ').populate('blogs')
        ...
        console.error('Error in getBlogByUser controller:', error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
}
...
module.exports = { addBlog, getBlogs, getOneBlog, updateBlog, getByUserId, fileUpload, deleteBlog }